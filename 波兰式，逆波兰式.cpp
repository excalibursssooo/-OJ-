...
void EvaluateExpression(string MyExp) //算术表达式求值算法
...
/********** Write your code here! **********/
void DSpoland(string MyExp) {
    stack<string> OPTR; //运算符栈，字符元素
    stack<string> OPND; //运算数栈，实数元素
    string Data, a, b, r;
    int pos = 0;
    string theta, c, x;
    string Dr;
    ...
    while (!OPTR.empty()) {
        if (!In(MyExp[i], OPSET)) {
            ...
            string jud = OPTR.top();
            if (precede(jud[0], MyExp[i]) == '<') {
                string p;
                p = p + MyExp[i] + " ";
                // cout<<"*";
                OPTR.push(p);
                i++;
            } else if (precede(jud[0], MyExp[i]) == '>') {
                a = OPND.top();
                OPND.pop();
                b = OPND.top();
                OPND.pop();
